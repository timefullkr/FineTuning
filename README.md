...
# 3. 파인튜닝하기
**TRAINING_FILE_ID**의 값은  데이터 파일 업로드에서 response.json() 값으로 넘어온 데이터
```
...
print(response.json())
```
파인튜닝이 정상적으로 종료되면 위의 모델 생성 요청 리턴 값 response.json() 과 메일로 **파인튜닝된 모델의 id**가 도착한다. 적게는 10분에서 몇 시간이 소요될 수 있다.
